...
function findAllPSiblings(where) {
    ...
    for (let element of where.children) {
        if (element.nextElementSibling !== null &&
            element.nextElementSibling.nodeName === 'P') {
            array.push(element);
        }
    }
...
function deleteTextNodesRecursive(where) {
    for (let node of where.childNodes) {
        ...
        // по всему содержимому этой ноды и сделала то же самое
        if (node.hasChildNodes()) {
            deleteTextNodesRecursive(node);
        }
        // когда зачистила там всё, по стеку выше идет в следующую ноду на
        // более высоком уровне вложенности
    }

    function clearTextNodes(where) {
        for (let node of where.childNodes) {
            if (node.nodeType === 3) {
                where.removeChild(node);
            }
